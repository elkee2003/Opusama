...
import StudentAccFeed from '../StudentAccPost'
...
const PostList = () => {
    ...
    try {
      ...
      const allPosts = await Promise.all(
        realtors.map(async (realtor) => {
          ...
          const filteredPosts = posts.filter((post) => post.propertyType === 'Student Accommodation');
  ...
  return (
    <View style={styles.container}>
      <View>
        {/* Search Bar */}
        <Link href={'search/studentaccommodationsearch'} asChild>
...

      {
        ...
        <FlatList 
            ...
            renderItem={({item})=> <StudentAccFeed post={item}/>}
            refreshControl={
              <RefreshControl
        ...
        />
        :
        <Text style={styles.noListings}>No Student Accommodation listings</Text>
      }
    </View>
  )
}
